...
const crypto = require('crypto'); // Ensure this is explicitly required
...
// 🎯 MONGO DB CONFIG
// NOTE: For deployment, replace the hardcoded URI below with process.env.MONGO_URI
...
// --- 5. FIREBASE ADMIN INITIALIZATION (FIXED FOR DEPLOYMENT) ---
try {
    const serviceAccountJson = process.env.FIREBASE_SERVICE_ACCOUNT;
    let serviceAccount;

    if (serviceAccountJson) {
        // Parse the JSON string from the environment variable (for Render/Heroku)
...
    console.error("❌ Firebase Admin Initialization Failed. Ensure 'FIREBASE_SERVICE_ACCOUNT' environment variable is set or 'firebase-adminsdk.json' is present.", error.message);
    // If Admin SDK fails, the server can't generate tokens, so log it as a severe error.
    // However, we allow the server to start to handle non-auth routes if needed.
}


// --- 6. FIREBASE/FIRESTORE SYNC UTILITY (No Change) ---
/**
...
// 🔑 RE-INCORPORATED: We need the verification code generation function back in server.js
...
// --- 8. MIDDLEWARE DEFINITIONS (No Change) ---
...
/**
 * POST /api/register 
 * 🎯 MODIFIED: Now returns token/code to client for SendGrid call.
 */
...
        
        // 🔑 NEW RESPONSE: Return all required data for client-side SendGrid call
        res.json({ 
            success: true, 
...
// --- 10. INITIALIZATION (Modified MongoDB URI read) ---
