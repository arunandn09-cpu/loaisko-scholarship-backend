...
// 9️⃣ NEW: ADMIN UPDATE RESUBMITTED FILES (Verification/Notes)
app.post('/api/admin/update-resubmission-files', verifyAdmin, async (req, res) => {
    // Expected body:
    // {
    //     applicationId: "...",
    //     documentType: "grades", // The document key to update
    //     verified: true/false,   // The new verification status
    //     adminNote: "..."        // The new admin comment
    // }
    const { 
        applicationId, 
        documentType, 
    ...
    if (!applicationId || !documentType || typeof verified === 'undefined') {
        return res.status(400).json({ 
            success: false, 
            message: "Missing Application ID, document type, or verification status." 
        });
    }

    try {
        const fileDocRef = firestoreDb.collection('resubmission_files').doc(applicationId);

        // Use Firestore's dot notation update to target nested fields efficiently.
        // This ensures only the specified document's fields are updated, and nothing else.
        const updatePayload = {
            [`documents.${documentType}.verified`]: verified,
            [`documents.${documentType}.adminNote`]: adminNote || null,
            updatedAt: admin.firestore.FieldValue.serverTimestamp()
        };

        await fileDocRef.update(updatePayload);

        res.json({ 
            success: true, 
            message: `Resubmitted document ${documentType} updated successfully for application ${applicationId}.` 
        });

    } catch (error) {
        console.error("💥 Failed to update resubmission document:", error);

        // Check for specific error codes (e.g., Firestore document not found)
        if (error.code === 5) { // NOT_FOUND
             return res.status(404).json({
                success: false,
                message: "Resubmission file document not found for the given Application ID."
            });
        }
        
        res.status(500).json({ 
            success: false, 
            message: "Server error during resubmission file update.",
...
app.delete('/api/admin/delete-student', verifyAdmin, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            // Deleting the resubmission document by UID is now less critical since it will be by App ID,
            // but we keep this for legacy/other collections where UID is the key.
